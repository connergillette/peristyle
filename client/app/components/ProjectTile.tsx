...
export default function ProjectTile({ month, projectName, progress, color, next }: { month: string, projectName?: string, progress?: number, color?: string, next?: boolean }) {
  ...
  return (
    <Link to={`/projects/${projectName?.toLowerCase()}`} className={`flex flex-col text-center shrink-0 pt-2 snap-always snap-center rounded-md ${color} text-white ${next ? ' border-dashed border-black border-4' : ''}`}>
      {month}
