...
export default function ProjectTile({ month, projectName, progress, color, next }: { month: string, projectName?: string, progress?: number, color?: string, next?: boolean }) {
  ...
  return (
    <a href={`/projects/${projectName?.toLowerCase()}`} className={`flex flex-col text-center w-1/6 shrink-0 pt-2 rounded-md ${color} text-white ${next ? ' border-dashed border-black border-4' : ''}`}>
      {month}
